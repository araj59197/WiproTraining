    ...
    def extract_tables(self, max_tables=5):
        ...
        if self.soup:
            ...
            for idx, table in enumerate(self.soup.find_all("table")[:max_tables]):
                table_data = {"table_id": idx + 1, "headers": [], "rows": []}
    ...
    def extract_lists(self, max_lists=10):
        ...
        if self.soup:
            ...
            for idx, ul in enumerate(self.soup.find_all("ul")[:max_lists]):
                list_items = [
                    li.text.strip() for li in ul.find_all("li", recursive=False)
                ]
                if list_items:
                    lists.append(
                        {"type": "unordered", "list_id": idx + 1, "items": list_items}
                    )

            # Extract ordered lists
            for idx, ol in enumerate(self.soup.find_all("ol")[:max_lists]):
                list_items = [
                    li.text.strip() for li in ol.find_all("li", recursive=False)
                ]
                if list_items:
                    lists.append(
                        {"type": "ordered", "list_id": idx + 1, "items": list_items}
                    )
